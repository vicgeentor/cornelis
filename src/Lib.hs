...
cornelis = do
  ...
  wrapPlugin $ Plugin
    { environment = env
    , exports =
        [ $(command "CornelisRestart"          'doRestart)          [CmdSync Sync]
        , $(command "CornelisAbort"            'doAbort)            [CmdSync Sync]
        , $(command "CornelisLoad"             'doLoad)             [CmdSync Sync]
        , $(command "CornelisGoals"            'doAllGoals)         [CmdSync Sync]
        , $(command "CornelisSolve"            'solveOne)           [CmdSync Sync, rw_complete]
        , $(command "CornelisAuto"             'autoOne)            [CmdSync Sync, rw_complete]
        , $(command "CornelisTypeInfer"        'doTypeInfer)        [CmdSync Sync]
        , $(command "CornelisTypeContext"      'typeContext)        [CmdSync Sync, rw_complete]
        , $(command "CornelisTypeContextInfer" 'typeContextInfer)   [CmdSync Sync, rw_complete]
        , $(command "CornelisMakeCase"         'doCaseSplit)        [CmdSync Sync]
        , $(command "CornelisRefine"           'doRefine)           [CmdSync Sync]
        , $(command "CornelisGive"             'doGive)             [CmdSync Sync]
        , $(command "CornelisElaborate"        'doElaborate)        [CmdSync Sync, rw_complete]
        , $(command "CornelisPrevGoal"         'doPrevGoal)         [CmdSync Sync]
        , $(command "CornelisNextGoal"         'doNextGoal)         [CmdSync Sync]
        , $(command "CornelisGoToDefinition"   'doGotoDefinition)   [CmdSync Sync]
        , $(command "CornelisWhyInScope"       'doWhyInScope)       [CmdSync Sync]
        , $(command "CornelisNormalize"        'doNormalize)        [CmdSync Sync, cm_complete]
        , $(command "CornelisHelperFunc"       'doHelperFunc)       [CmdSync Sync, rw_complete]
        , $(command "CornelisQuestionToMeta"   'doQuestionToMeta)   [CmdSync Sync]
        , $(command "CornelisInc"              'doIncNextDigitSeq)  [CmdSync Sync]
        , $(command "CornelisDec"              'doDecNextDigitSeq)  [CmdSync Sync]
        , $(command "CornelisAgdaVersion"      'doGetVersion)       [CmdSync Sync]
        , $(command "CornelisDebug"            'doDebug)            [CmdSync Sync, debug_complete]
        ...
        , $(function "InternalCornelisNotifyEdit" 'notifyEdit) Sync
        ]
    }
